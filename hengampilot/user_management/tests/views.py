...
class UserViewSetTest(APITestCase):
    ...
    def test_fetch_user_by_username_success(self):
        ...
        url = reverse('user_management:url_user_view-fetch-by-username') + '?username=testuser'  # Use the proper name
    ...
    def test_fetch_user_by_username_not_found(self):
        ...
        url = reverse('user_management:url_user_view-fetch-by-username') + '?username=nonexistentuser'  # Use the proper name
        self.client.credentials(HTTP_AUTHORIZATION='Bearer ' + self.token)
        response = self.client.get(url, format="json")
    ...
    def test_fetch_user_by_username_missing_param(self):
        ...
        url = reverse('user_management:url_user_view-fetch-by-username')  # Use the proper name
        self.client.credentials(HTTP_AUTHORIZATION='Bearer ' + self.token)
        response = self.client.get(url, format="json")
...
class NotificationViewSetTest(APITestCase):
    ...
    def test_fetch_notifications(self):
        ...
        url = reverse('user_management:url_notification_view-list')  # Use the proper name
        self.client.credentials(HTTP_AUTHORIZATION='Bearer ' + self.token)
        response = self.client.get(url, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
        self.assertEqual(response.data[0]['notofication_text'], "This is a test notification")
    ...
    def test_notification_is_associated_with_user(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION='Bearer ' + self.token)
        response = self.client.get(url, format="json")
        self.assertEqual(str(response.data[0]['user_notifications']), str(self.user.id))  # Check if the notification is related to the current user
